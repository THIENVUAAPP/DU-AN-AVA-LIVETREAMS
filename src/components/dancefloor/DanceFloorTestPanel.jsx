...
import { PlayCircle, Radio, Send, Gift, Youtube, Music2, Facebook, Wifi, WifiOff, Loader2, UserRound, Volume2, VolumeX, Shuffle, Clock, MonitorPlay, RadioTower } from 'lucide-react';
...
export default function DanceFloorTestPanel({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass-panel p-4 rounded-2xl border border-white/10">
        ...
        <div className="flex items-center justify-between gap-3 mt-3 pt-3 border-t border-white/10">
          <div className="min-w-0">
            ...
            <p className="text-[9px] text-gray-500">Dùng đúng tài khoản/stream key đã kết nối sẵn ở trên — không cần đăng nhập lại.</p>
          </div>
          <button
            onClick={onToggleLive}
            disabled={connectedChannels.length === 0}
            className={`px-4 py-2.5 rounded-xl text-xs font-black cursor-pointer shrink-0 flex items-center gap-1.5 transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
      ...
      {/* Tự động hoá: giọng đọc, phong cách bình luận, Auto-Shuffle, lịch 24/7, cửa sổ overlay */}
      <div className="glass-panel p-4 rounded-2xl border border-white/10 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold text-gray-300 flex items-center gap-1.5">
            ...
            Giọng Đọc Bình Luận (miễn phí, trình duyệt)
          </span>
          <button
