...
export default function CleanLiveOverlay({ customStyle = {} }) {
  ...
  const blobCacheMapRef = useRef(new Map());
  const preloadingUrlRef = useRef(null);
  ...
  // 👥 MULTI-AVATAR LIVE STUDIO (2–4 NHÂN VẬT) REALTIME SYNC
  ...
  useEffect(() => {
    ...
    const handleVideoPlaybackControl = (control) => {
      ...
      if (action === 'pause' || (control.userPaused === true && isPlaying === false)) {
        isUserPausedRef.current = true;
      ...
      } else if (action === 'play' || (action !== 'time_sync' && isPlaying === true)) {
        isUserPausedRef.current = false;
      ...
      <main className="absolute inset-0 w-full h-full overflow-hidden flex items-center justify-center bg-black z-0">
        ...
        >
          ...
          {currentStage === 'idol' && (
            ...
            {/* MULTI-AVATAR STUDIO (2-4 CHARACTERS) */}
