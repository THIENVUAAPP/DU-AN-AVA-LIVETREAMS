...
export default function GameBanDoVietNam({
  ...
  const [autoRotate, setAutoRotate] = useState(() => bandoEngine.state.autoRotate !== undefined ? bandoEngine.state.autoRotate : false);
  ...
  // Handle external system commands if passed from parent
  useEffect(() => {
    if (!externalLiveEvent) return;
    const { type } = externalLiveEvent;
    if (type === 'RESET') {
      bandoEngine.resetRound();
    } else if (type === 'BOSS') {
