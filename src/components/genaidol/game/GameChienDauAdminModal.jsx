...
export default function GameChienDauAdminModal({ 
  ...
  const handleCopyOverlayLink = () => {
    if (typeof window !== 'undefined') {
      const url = `${window.location.origin}/?overlay=gamebattle`;
            ...
            <div className="p-4 overflow-y-auto max-h-[56vh] space-y-4">
              ...
              {activeTab === 'commentary' && (
                <div className="space-y-3">
                  <div className="p-3.5 bg-gradient-to-r from-pink-950/40 to-purple-950/40 border border-pink-500/40 rounded-xl space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="text-xs font-black text-pink-300 uppercase flex items-center gap-1.5">
                          <Mic size={14} className="text-pink-400 animate-pulse" /> Bộ Não AI & Bình Luận Viên Trực Tiếp
                        </h4>
                        <p className="text-[10px] text-gray-300 mt-0.5">
                          AI tự động bình luận, cổ vũ theo chu kỳ và phản hồi ngay lập tức khi có người tặng quà, tung tuyệt kỹ.
                        </p>
                      </div>
                      <button
                        onClick={() => {
                          const updated = { ...config, commentaryEnabled: !config.commentaryEnabled };
                          setConfig(updated);
                          battleCommentary.isEnabled = updated.commentaryEnabled;
                          battleCommentary.saveSettings();
                        }}
                        className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all ${
                          config.commentaryEnabled !== false 
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 border-t border-pink-500/20">
                        ...
                        <div className="flex items-center gap-1.5">
                          {[10, 15, 20, 30, 60].map(sec => (
                            <button
                              key={sec}
                              onClick={() => {
                                setConfig({ ...config, commentaryInterval: sec });
                                battleCommentary.intervalSeconds = sec;
                                battleCommentary.saveSettings();
                              }}
                              className={`px-2 py-1 rounded-lg text-[10px] font-mono font-bold transition-all ${
                                (config.commentaryInterval || 15) === sec 
                                  ? 'bg-pink-600 text-white border border-pink-400 shadow-sm' 
                                  : 'bg-black/50 text-gray-400 hover:text-white border border-white/10'
                              }`}
                            >
                              {sec}s
                      ...
                      {/* AI Voice Volume */}
                      <div>
                        <div className="flex justify-between text-[11px] font-bold text-gray-200 mb-1">
                          <span>Âm lượng Giọng đọc AI:</span>
                          <span className="font-mono text-pink-400">{Math.round((config.commentaryVolume !== undefined ? config.commentaryVolume : 0.9) * 100)}%</span>
                        </div>
                        <input
                          type="range"
                          min="0"
                          max="1"
                          step="0.05"
                          value={config.commentaryVolume !== undefined ? config.commentaryVolume : 0.9}
                          onChange={(e) => {
                            const val = parseFloat(e.target.value);
                            setConfig({ ...config, commentaryVolume: val });
                            battleCommentary.volume = val;
                            battleCommentary.saveSettings();
                          }}
                    ...
                    <div className="flex items-center justify-between pt-1 border-t border-pink-500/20">
                      <span className="text-[10px] text-pink-300 flex items-center gap-1">
                        ...
                        Tự động hạ nhỏ nhạc nền khi AI phát ngôn (Audio Ducking)
                      </span>
                      <button
                  ...
                  {/* KỊCH BẢN & CÂU NÓI TÙY BIẾN */}
                  <div className="p-3.5 bg-black/40 border border-white/10 rounded-xl space-y-3">
                    <h4 className="text-xs font-bold text-gray-200 uppercase flex items-center justify-between">
                      <span>Danh Sách Câu Nói & Kịch Bản AI Tự Động Bình Luận</span>
                      <span className="text-[10px] text-gray-400 font-mono">
                        {battleCommentary.customPrompts.length} câu đã lưu
                      </span>
                    </h4>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onKeyDown={(e) => e.key === 'Enter' && addPromptScript()}
                        placeholder="Nhập câu nói kích thích người xem (Ví dụ: Anh em phe Đỏ mau tặng quà để lật kèo!)..."
                      ...
                      />
                      <button
                        onClick={addPromptScript}
                        className="px-3 py-1.5 bg-pink-600 hover:bg-pink-500 text-white rounded-lg text-xs font-bold flex items-center gap-1 shadow shrink-0"
                      >
                    ...
                    <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                      {battleCommentary.customPrompts.map((prompt, idx) => (
                        <div key={idx} className="p-2 bg-white/5 border border-white/5 rounded-lg flex items-center justify-between gap-2 text-xs">
                          <span className="text-gray-200 text-[11px] leading-snug flex-1">
                            {idx + 1}. "{prompt}"
                          </span>
                          <div className="flex items-center gap-1 shrink-0">
                            <button
                              onClick={() => testVoiceSpeech(prompt)}
                              className="p-1 rounded bg-pink-500/20 text-pink-300 hover:bg-pink-500/30"
                              title="Nghe thử câu này"
                            >
                              <Play size={11} />
                            </button>
                            <button
                              onClick={() => removePromptScript(idx)}
