...
import { Download, Monitor, ArrowRight, UserSquare2, Cpu, MessageSquareText, Package, Plus } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-6 text-white pb-24">
      ...
      <div className="bg-[#121216]/80 backdrop-blur-md rounded-2xl p-8 flex flex-col md:flex-row gap-8 justify-between relative overflow-hidden border border-white/10 shadow-lg">
         ...
         <div className="flex-1 relative z-10">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-xs font-bold text-white shadow-sm">
                <span className="text-[#00FF66]">01</span> Chuẩn bị trên web
              </div>
              <ArrowRight className="w-4 h-4 text-gray-600" />
              <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-xs font-bold text-white shadow-sm">
                <span className="text-[#00FF66]">02</span> Đồng bộ về app
              </div>
              <ArrowRight className="w-4 h-4 text-gray-600" />
              <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-full text-xs font-bold text-white shadow-sm">
                <span className="text-[#00FF66]">03</span> Phát phiên live
              </div>
            </div>
         </div>
         ...
         <div className="w-full md:w-[380px] relative z-10 flex flex-col gap-3">
            ...
            <button className="flex items-center gap-4 p-4 rounded-xl bg-black/40 border border-white/10 hover:bg-white/5 hover:border-[#00FF66]/50 transition-all group text-left">
               <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#00FF66] to-teal-600 flex items-center justify-center shadow-lg shadow-glow-green">
                 <Package className="w-5 h-5 text-black" />
               </div>
               <div className="flex-1">
                 <div className="text-sm font-bold text-white mb-0.5 group-hover:text-[#00FF66] transition-colors">Tải AIDOL AutoPin</div>
                 <div className="text-[10px] text-gray-400">Tự động ghim sản phẩm khi đang live</div>
               </div>
               <div className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center text-gray-400 group-hover:bg-[#00FF66] group-hover:text-black transition-colors">
                 <Download className="w-3 h-3" />
               </div>
            </button>
            
            <div className="text-right mt-2">
              <a href="#" className="text-[10px] font-bold text-[#00FF66] hover:text-[#00CC52]">Hoặc quản lý thư viện AIDOL →</a>
            </div>
         </div>
      </div>
      ...
      <div>
         ...
         {/* Step Content */}
         <div className="bg-[#121216]/60 backdrop-blur-md rounded-2xl border border-white/10 p-8 shadow-lg">
            {activeStep === 1 && (
              <div>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
                  <div>
                    ...
                    <h3 className="text-2xl font-black text-white mb-2">Chọn AIDOL trước khi setup phiên Live</h3>
                    <p className="text-sm text-gray-400 font-medium max-w-xl">AIDOL là hồ sơ gốc của phiên. Bộ não là một prompt duy nhất, lưu đi kèm nhân vật và dùng lại cho mọi kịch bản.</p>
                  </div>
                  <div className="bg-black/40 p-4 rounded-xl border border-white/10 shadow-sm max-w-xs">
                     <h4 className="font-bold text-[#00FF66] text-sm mb-1">5 AIDOL / tài khoản</h4>
                     <p className="text-[10px] text-gray-400">Khi chưa có nhân vật, chỉ cần thêm một ảnh để tạo AIDOL mới.</p>
                  </div>
                </div>

                <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-6">
                  <div className="flex items-center gap-2">
                    <h4 className="font-bold text-white text-sm">AIDOL đã lưu</h4>
                    <span className="text-[10px] text-gray-500">Chưa có AIDOL nào. Tạo nhanh từ một ảnh, sau đó quay lại chọn nhân vật vừa tạo.</span>
                  </div>
                  <button className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 hover:border-[#00FF66] hover:text-[#00FF66] text-gray-300 rounded-lg text-sm font-bold shadow-sm transition-colors">
                    <Plus className="w-4 h-4" /> Tạo AIDOL từ ảnh
                  </button>
                </div>

                <div className="py-16 text-center">
                  <h3 className="text-xl font-black text-white mb-2">Chưa có AIDOL để chọn</h3>
                  <p className="text-sm text-gray-500 font-medium">Thêm ảnh nhân vật để xem AIDOL hoặc tạo nhanh một phiên bản ảo từ ảnh nguồn của bạn.</p>
                </div>
              </div>
            )}
            ...
            {activeStep !== 1 && (
              <div className="py-24 text-center">
                 <div className="w-16 h-16 rounded-2xl bg-black/40 flex items-center justify-center mx-auto mb-4 border border-white/10">
                    <UserSquare2 className="w-8 h-8 text-[#00FF66]" />
                 </div>
                 <h3 className="text-xl font-black text-white mb-2">Vui lòng chọn nhân vật trước</h3>
                 <p className="text-sm text-gray-400 font-medium max-w-sm mx-auto">Bạn cần tạo và chọn một AIDOL ở Bước 1 trước khi cấu hình Bộ não AI và Nội dung.</p>
              </div>
            )}
         </div>
      </div>
