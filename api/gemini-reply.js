...
function buildPrompt({ kind, username, characterName, giftName, personality, question, role, context, gameType }) {
  ...
  if (kind === 'gift') {
    ...
    return `Bạn là MC / Idol dẫn chương trình livestream giải trí trên TikTok tại Việt Nam, phong cách ${tone}. Một khán giả tên "${safeUsername}" vừa tặng quà "${safeGift}". Viết 1 câu cảm ơn thật tự nhiên, chân thành, xưng "em" với "bạn" (ví dụ: "Em cảm ơn bạn ${safeUsername}..."), QUY TẮC BẮT BUỘC: Bạn luôn xưng là "em" và gọi người xem là "bạn". KHÔNG quá 25 từ, KHÔNG dùng dấu ngoặc kép, tuyệt đối KHÔNG nói tục, KHÔNG nói bậy. Chỉ trả về đúng 1 câu thoại tiếng Việt, không giải thích gì thêm.`;
  }
  
  if (kind === 'welcome') {
    return `Bạn là MC / Idol dẫn chương trình livestream trên TikTok tại Việt Nam, phong cách ${tone}. Một khán giả tên "${safeUsername}" vừa vào xem và bình luận lần đầu. Viết 1 câu chào mừng thật tự nhiên, xưng "em" với "bạn" (ví dụ: "Dạ em chào bạn ${safeUsername}..."), QUY TẮC BẮT BUỘC: Bạn luôn xưng là "em" và gọi người xem là "bạn". KHÔNG quá 25 từ, KHÔNG dùng dấu ngoặc kép, tuyệt đối KHÔNG nói tục, KHÔNG nói bậy. Chỉ trả về đúng 1 câu thoại tiếng Việt, không giải thích gì thêm.`;
  }
